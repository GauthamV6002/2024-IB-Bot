	...
	async execute(client, member) {
		...
		await member.send(
			client.simpleEmbed(
				...
				{
					thumbnail: {
						url: "https://yt3.ggpht.com/ytc/AKedOLRtCl5_ppqfinTZHIDjo9VpJqGeTPUhY3Iq9EmH=s900-c-k-c0x00ffffff-no-rj",
					},
				}
			)
		);
		...
		client.on("interactionCreate", async (i) => {
			...
			if (i.customId === `select-full-or-partial-${programmeRand}`) {
				...
				const { status, msg } = checkCourses(
					courses,
					role_names[0] === "Full IB"
				);
				if (status) {
					i.update(
						client.simpleEmbed(
					...
					const cacheMember = guild.members.cache.get(
						member.id
					);
					await cacheMember.roles.remove(
						cacheMember.roles.cache.filter((r) =>
							all_roles.includes(r.name)
						)
					);
					await cacheMember.roles.add(roles);
				} else {
					const coursesRow = getCoursesRow(
						role_names[0],
						coursesRand
					);
					i.update(
