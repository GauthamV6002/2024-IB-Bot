...
const checkCourses = (courses, isfullIB) => {
	...
	for (conflict of conflicts) {
		...
		if (common.length > 1) {
			return {
				status: false,
				msg: `❌ Conflicts detected! (Between: ${conflict.join(", ")})`,
			};
		}
	}
	...
	if (isfullIB) {
		...
		if (!courses.includes("ELA HL")) {
			return {
				status: false,
				msg: "❌ Full IB students must take ELA HL.",
			};
		} else if (
			!courses.includes("Math HL") &&
			!courses.includes("Math SL")
		) {
			return {
				status: false,
				msg: "❌ Full IB students must choose either Math SL or HL.",
			};
		} else if (
			courses.filter((c) => language_roles.includes(c)).length !== 1
		) {
			return {
				status: false,
				msg: "❌ Full IB students must take 1 language.",
			};
		} else if (sciences.length > 2 || sciences.length < 1) {
			return {
				status: false,
				msg: "❌ Full IB students must take 1-2 sciences (Bio, Chem, Physics, Comp Sci).",
			};
		} else if (socials.length > 2 || socials.length < 1) {
			return {
				status: false,
				msg: "❌ Full IB students must take 1-2 courses in the social studies category. (World History HL, Economics HL, and Business Management SL).",
			};
...
module.exports = {
	...
	async execute(interaction, client) {
		if (interaction.user.id !== "901273742901133393"){
			interaction.reply({ content: "This command is only meant for bot developer(s).", ephemeral: true });
			return;
		}

		const guild = client.guilds.cache.get(guildId);
		const members = guild.members.cache;
		const invalidMembers = members.filter(m => invalids.includes(`${m.user.username}#${m.user.discriminator}`));
